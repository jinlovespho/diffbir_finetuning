...
def load_pair_list(file_list_path: str) -> List[Dict[str, str]]:
    default_data_type = ['lsdir', 'realsr'] # TODO: change to config
    base_dir = '/home/cvlab12/project/jaewon/text_restoration/data_gen/ocr'
    files = []
    for data_type in default_data_type:
        for mode in ['train', 'val']:
            ...
            for img_path in os.listdir(os.path.join(file_list_path, data_type, mode, 'gt/512')):
                ...
                try:
                    prompt = json_data[img_id][idx]['text']
                except:
                    prompt = ''
                ...
                if not os.path.exists(lq_path):
                    continue
                files.append({"image_path": gt_path, "lr_image_path": lq_path, "prompt": f'The word "{prompt}" is written on the image.'})
