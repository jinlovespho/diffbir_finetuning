...
class Diffusion(nn.Module):
    ...
    def p_losses(self, model, z_0, t, cond):
        '''
            z_0: (b 4 56 56) -> vae encoded gt(hr) img
            x_noisy: (b 4 56 56) -> noise depending on t, added to latent

        '''
        noise = torch.randn_like(z_0)
        z_t = self.q_sample(x_start=z_0, t=t, noise=noise)  # q: forward process: adding noise to img: x_t = x0 + noise_t 
        model_output = model(z_t, t, cond)  # model outputs noise_t

        if self.parameterization == "x0":
            target = z_0
        elif self.parameterization == "eps":    # t 
            target = noise
        ...
        loss_simple = self.get_loss(model_output, target, mean=False).mean()    # 아 내 말이 맞네! 바로 model_output 과 target=noise 사이 loss를 주네 

        return loss_simple, pred_z_0, z_t
